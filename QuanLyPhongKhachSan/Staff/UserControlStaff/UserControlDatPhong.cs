...
namespace QuanLyPhongKhachSan.Staff.UserControlStaff
{
    public partial class UserControlDatPhong : UserControl
    {
        ...
        private Guna2Panel TaoPhongMoi(Phong phong)
        {
            ...
            // Hiện khách (mới nhất chưa trả, kể cả tương lai)
            HienKhachLenPanel(phong, lblKhach, pnl);

            // Click 1 lần: chọn/bỏ chọn
            pnl.MouseDown += (s, e) =>
            {
        ...
        {
            var dat = phongService.LayDatPhongTheoMaPhong(maPhong); // TOP 1 ORDER BY NgayNhan DESC
            if (dat == null) return null;
            if (dat.NgayTraThucTe.HasValue) return null; // đã trả rồi thì không hiển thị
            return dat; // không lọc theo today nữa
        }
                ...
                using (var frmthemvasua = new frmThemvaSuaKH(phong))
                {
                    ...
                    if (dr == DialogResult.OK)
                    {
                        ...
                        if (maKh > 0)
                        {
                            ...
                            if (maDatPhong > 0)
                            {
                                // Cập nhật trực tiếp panel vừa thao tác
        ...
        private void btnThemKH_Click(object sender, EventArgs e)
        {
            ...
            // Build danh sách thông tin phòng + booking hiện tại (mới nhất chưa trả)
            var list = new List<RoomBookingInfo>();
            foreach (var room in selected)
            {
                var bk = GetDatPhongDeHien(room.MaPhong); // << dùng hàm mới nhất (mới nhất chưa trả)
            ...
            // Nếu có nhiều khách khác nhau -> không cho mở
            if (maKhSet.Count > 1)
            {
            ...
            if (maKhSet.Count == 1)
            {
                var any = bookedItems.First(); // cùng 1 MaKH nên bản nào cũng được
                var kh = any.Customer;
                if (kh != null)
                {
                    preTen = kh.HoTen ?? "";
        ...
        private void XoaPhong(Phong phong, Guna2Panel pnl)
        {
            ...
            _selectedRoomIds.Remove(phong.MaPhong);
        }
