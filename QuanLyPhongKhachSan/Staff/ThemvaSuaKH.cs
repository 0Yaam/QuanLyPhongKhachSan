...
namespace QuanLyPhongKhachSan
{
    public partial class frmThemvaSuaKH : Form
    {
        ...
        private bool _printedLan1 = false; // Ngăn in lần 1 nhiều lần
        ...
        public frmThemvaSuaKH(Phong phong)
        {
            ...
            LoadLoaiPhong(); // sẽ gán _giaPhong nếu có loại phòng

            // FIX: Giá bị ghi đè 0 do _phong.LoaiPhong chưa gán -> dùng SelectedItem nếu cần
            var selectedLoai = cbLoaiPhong.SelectedItem != null ? cbLoaiPhong.SelectedItem.ToString() : _phong.LoaiPhong;
            if (_phong.Gia > 0)
            {
                _giaPhong = _phong.Gia;
            }
            else
            {
                _giaPhong = LayGiaTheoLoaiPhong(selectedLoai);
            }
            if (!string.IsNullOrEmpty(selectedLoai)) _phong.LoaiPhong = selectedLoai; // đồng bộ object
            ...
            TryDisablePrintIfAlreadyPrinted();
        }

        private void TryDisablePrintIfAlreadyPrinted()
        {
            try
            {
                int maDat = _maDatHienTai;
                if (maDat <= 0)
                {
                    var dat = phongService.LayDatPhongTheoMaPhong(_phong.MaPhong);
                    if (dat != null) maDat = dat.MaDat;
                }
                if (maDat > 0)
                {
                    var list = _hoaDonService.LayDanhSach();
                    if (list.Any(x => x.MaDat == maDat && string.Equals(x.LoaiHoaDon, "Lần 1", StringComparison.OrdinalIgnoreCase)))
                    {
                        _printedLan1 = true;
        ...
        private decimal LayGiaTheoLoaiPhong(string loai)
        {
            ...
            return phongService.LayGiaTheoLoai(loai);
        }
        ...
        private void cbLoaiPhong_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            bool capNhatOk = phongService.CapNhat(_phong);
            if (!capNhatOk)
            {
                System.Diagnostics.Debug.WriteLine($"cbLoaiPhong_SelectedIndexChanged: CapNhat thất bại MaPhong={_phong.MaPhong}, LoaiPhong={tenLoai}");
            }
        }
        ...
        private void btnHoanThanh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Thông báo thay đổi khách hàng để UI khác reload
                AppEvents.RaiseCustomerChanged();
        ...
        private void btnInHoaDon_Click(object sender, EventArgs e)
        {
            try
            {
                if (_printedLan1)
                {
                    MessageBox.Show("Đã in Hóa đơn Lần 1 trước đó. Không thể in lại.");
                    return;
                }
                ...
                // Không cho in lần 1 nếu đã tồn tại trước đó với MaDat hiện tại
                TryDisablePrintIfAlreadyPrinted();
                if (_printedLan1)
                {
                    MessageBox.Show("Đã tồn tại Hóa đơn Lần 1 cho đặt phòng này.");
                    return;
                }
                ...
                // Sau khi in lần 1, không cho in nữa
                _printedLan1 = true;
                if (btnInHoaDon != null) btnInHoaDon.Enabled = false;
        ...
        private void ClearForm(bool clearLoai = true)
        {
